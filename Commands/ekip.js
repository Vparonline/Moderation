...
module.exports.execute = async (client, message, args, params) => {
    ...
    let embed = new Discord.MessageEmbed().setColor("RANDOM").setFooter(ayar.GenelAyarlar.setFooter, message.author.avatarURL({dynamic: true})).setTimestamp().setAuthor(message.guild.name, message.guild.iconURL({dynamic: true}))
    ...
    Katılım Tarihi: \`${new Date(value.KatılmaTarihi).toTurkishFormatDate()}\`
    ...
    **Detaylı bilgiler:**
    ...
    \`Ekibin sunucuya katılma tarihi:\` ${new Date(Date.now()).toTurkishFormatDate()}
    \`Ekip Rolü:\` <@&${role.id}>
